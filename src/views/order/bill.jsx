...
import { reqPayment } from '@/api/order';
...
class roleUser extends Component {
  ...
  getOrder = async () => {
    const result = await reqPayment(this.state.listQuery)
          ...
          <Panel header="筛选" key="1">
            <Form layout="inline">
              <Form.Item label="订单ID:">
                <Input onChange={this.filterUserIdChange} allowClear placeholder="请输入订单ID" />
              </Form.Item>
              <Form.Item label="交易号:">
                <Input onChange={this.filterUserIdChange} allowClear placeholder="请输入交易号" />
              </Form.Item>
